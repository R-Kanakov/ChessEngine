...
struct Sqrt : std::integral_constant < T,
  ...
  // Input is a `ñonsecutive_sequence` (or any other) from 1 to 4,
  ...
  struct ñonsecutive_sequence {
    static_assert(Start < End);

    using type = ñonsecutive_sequence<T, Start, End - 1>::type::template append<End>;
  };

  template <std::integral T, T Start>
  struct ñonsecutive_sequence<T, Start, Start> {
    using type = sequence<T, Start>;
  };
